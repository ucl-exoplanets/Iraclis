            ...
            if self.hdu_image:
                ...
                if self.dark_mode:
                    ...
                    sample_numbers = [input_data[pp].header[variables.sample_number.keyword]
                                      for pp in functions.sci(input_data)]

                    for i in functions.sci(selected_calibration_data):
                    ...
                    crop1 = int(len(np.array(array)) / 2 - len(input_data[functions.sci(input_data)[0]].data) / 2)
                    crop2 = int(len(np.array(array)) / 2 + len(input_data[functions.sci(input_data)[0]].data) / 2)
