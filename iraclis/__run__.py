...
def process_visit(list_of_parameters_files):

    if isinstance(list_of_parameters_files, str):
        list_of_parameters_files = [list_of_parameters_files]

    for parameters_file in list_of_parameters_files:
            ...
            for sample in range(total_samples):
                ...
                for i in range(len(raw_data_set.spectroscopic_images)):
                    ...
                    new_frame = [pf.PrimaryHDU(header=frame[0].header, data=frame[0].data),
                                 ...
                                 pf.ImageHDU(header=frame['SKYAREA'].header, data=frame['SKYAREA'].data, name='SKYAREA'),
                                 pf.ImageHDU(header=frame['SCANMAP'].header, data=frame['SCANMAP'].data, name='SCANMAP'),
                                 ...
                                 pf.ImageHDU(header=frame['BPCRMAP'].header, data=frame['BPCRMAP'].data, name='BPCRMAP'),
                                 ]
        ...
        if pipeline_variables.fitting_white.value or pipeline_variables.fitting_spectrum.value:

            light_curve = open_lightcurve(lc_file)

            if pipeline_variables.fitting_white.value:

                light_curve = fitting(light_curve, fitting_spectrum=pipeline_variables.fitting_spectrum.value)
